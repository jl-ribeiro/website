export const allProjects = {
  'otimizacao-dataform': {
    ...
    pt: {
      ...
      subtitle: 'Otimização de 8x na velocidade de atualização de um Data Warehouse legado.',
      summary: 'Este projeto aborda a reestruturação completa de um Data Warehouse no BigQuery, migrando um pipeline legado e custoso de dbt para uma solução nativa e eficiente com Google Dataform.',
      challenge: ['A arquitetura anterior, baseada em dbt e Airflow, era insustentável, com custos de processamento elevados e desnecessários.', 'Muitas tabelas não eram mais utilizadas, mas ainda consumiam recursos.', 'O tempo de atualização de 40 minutos para o Data Warehouse completo impactava a agilidade do negócio.'],
      ...
      results: ['Redução de 8x no tempo de atualização do Data Warehouse (de 40 para 5 minutos).', 'Custos de processamento drasticamente reduzidos devido à eliminação de tabelas obsoletas e processos otimizados.', 'Melhora significativa na governança e manutenibilidade da base de dados.'],
    },
    en: {
      ...
      techDeepDive: [
        {
          ...
          description: 'By adopting Dataform, we eliminated the dependency on Airflow to orchestrate dbt, leveraging BigQuery\'s native and free integration. This simplified the stack and drastically accelerated the update time from 40 to less than 5 minutes.'
        },
        {
          title: 'Modularization with JavaScript',
          description: 'Business rules that were repeated across multiple models were encapsulated in JavaScript functions, imported directly into SQLX files. This increased maintainability and reduced code duplication.'
        }
      ],
      results: [
  ...
  'ingestao-cdc': {
    ...
    pt: {
      ...
      subtitle: 'Criação de um pipeline de replicação de dados near real-time com redução de 98% nos custos.',
      summary: 'Desenvolvi uma arquitetura de ingestão de dados utilizando Change Data Capture (CDC) para replicar dados de um banco transacional (MySQL) para um ambiente analítico (BigQuery) de maneira rápida, confiável e econômica. A solução foi projetada para minimizar o tempo de latência e custos operacionais, oferecendo uma solução robusta e escalável.',
      challenge: [
